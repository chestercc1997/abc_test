...
void Vec_IntPrint1(Vec_Int_t *v) {
    ...
    int size = Vec_IntSize(v);
    int *array = Vec_IntArray(v);

    printf("[");
    for (int i = 0; i < size; i++) {
        printf("%d", array[i]);
        if (i < size - 1) { // 仅在不是最后一个元素时打印逗号
            printf(", ");
        }
    }
    printf("]");
...
void Vec_IntPrintToFile(FILE *file, Vec_Int_t *vec) {
    ...
    fprintf(file, "[");
    for (int i = 0; i < Vec_IntSize(vec); i++) {
        if (i > 0) {
            fprintf(file, ", "); // 添加逗号分隔符
        }
        fprintf(file, "%d", Vec_IntEntry(vec, i)); // 写入向量中的每一个元素
    }
    fprintf(file, "]"); // 闭合 JSON 数组
}
/**Function*************************************************************
